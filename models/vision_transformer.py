...
from models.weight_init import  trunc_normal_, lecun_normal_
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape # shape (batch, 3, 224, 224)
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."

        # flatten: [B, C, H, W] -> [B, C, HW]
        # transpose: [B, C, HW] -> [B, HW, C]
        x = self.proj(x) # shape (batch, 192, 14, 14) 224 / 16 = 14
        x = x.flatten(2).transpose(1, 2) # shape (batch, 196, 192)
        x = self.norm(x)
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape # x shape (batch, 197, 192)
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)  # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn_weights = attn.softmax(dim=-1)  # 保存未经dropout处理的注意力权重
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # 对blocks进行迭代，收集每个block的注意力权重
        for blk in self.blocks:
            x, attention_weights = blk(x)
            attn_weights_list.append(attention_weights)

        self.attentions = attn_weights_list
        x = self.norm(x)
        if self.dist_token is None:
...
def _create_vision_transformer(variant,img_size=224, pretrained=False, default_cfg=None, **kwargs):
    ...
    if repr_size is not None and num_classes != default_num_classes:
        ...
        repr_size = None
    print(default_cfg)
    model = VisionTransformer(img_size=img_size,patch_size=kwargs['patch_size'],embed_dim=kwargs['embed_dim'],depth=kwargs['depth'],num_heads=kwargs['num_heads'],num_classes=num_classes)
    if pretrained:
        url= default_cfg.get('url', None)
